...
def cocktography_cb(word, word_eol, userdata, attr):
    if re.search(r"{}.*{}".format(START,STOP),word[1]):
        xchat.emit_print("Channel Message",'\0034\002\037' + word[0] + '\0034\002\037',dechoder(word[1]),"")
    ...
    if re.search(r"{}.*{}".format(MARK,STOP),word[1]):
        buffer[word[0]] = buffer[word[0]] + ' ' + word[1]
        xchat.emit_print("Channel Message",'\0034\002\037' + word[0] + '\0034\002\037',dechoder(buffer[word[0]]),"")
        del buffer[word[0]]
        return xchat.EAT_xchat
...
for event in events:
    xchat.hook_print_attrs(event, cocktography_cb)
...
def enchode_cb(word, word_eol, userdata):
    ...
    del buffer["input"]
    xchat.emit_print("Channel Message",'\0034\002\037' + xchat.get_info('nick') + '\0034\002\037',word_eol[1],"")
    
    return xchat.EAT_NONE

def dechode_cb(word, word_eol, userdata):
    xchat.prnt(dechoder(word[1])) 
    return xchat.EAT_NONE
