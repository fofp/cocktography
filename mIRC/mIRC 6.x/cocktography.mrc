;Secret Dechoder Ring v2.2.1 for mIRC 6.21 by Vin
;
;
...
on ^1:text:*:#:{
  ...
  if ($left(%message,6) === $hget(cocktography,start)) {
    if ($gettok(%message,$numtok(%message,32),32) == $hget(cocktography,cont)) {
      ...
      set %pen. $+ $nick $+ . $+ $chan $dechode(%m2)
      halt
    }
    if ($gettok(%message,$numtok(%message,32),32) == $hget(cocktography,stop)) {
      echo -t $chan <4 $+ $nick $+ > $decode($decode($dechode($left($right(%message,$calc($len(%message) - 7)),$calc($len(%message)-14))),m),m)
      halt
    }
  }
  if ($left(%message,4) === 8wmD)  {
    var %m2 = $left($right(%message,$calc($len(%message) - 5)),$calc($len(%message)-12))
    set %pen. $+ $nick $+ . $+ $chan $eval($+(%,pen,.,$nick,.,$chan),2) $+ $dechode(%m2)
    if ($gettok(%message,$numtok(%message,32),32) == $hget(cocktography,stop)) {
      echo -t $chan <4 $+ $nick $+ > $decode($decode($eval($+(%,pen,.,$nick,.,$chan),2),m),m)
      ;unset %pen. $+ $nick $+ . $+ $chan
      halt
    }
    halt
