/**
 * Cocktography IRCCloud Content Script (Chrome)
...
async function initCocktography() {
    try {
        // Load dicktionaries
        const baseURL = chrome.runtime.getURL('dicktionaries/');

        const [kontolRes, thinRes, wideRes] = await Promise.all([
            fetch(baseURL + 'kontol_chodes.txt'),
            fetch(baseURL + 'cock_bytes.txt'),
            fetch(baseURL + 'rodsetta_stone.txt')
        ]);

        const dicktionaries = {
            kontol_chodes: await kontolRes.text(),
            cock_bytes: await thinRes.text(),
            rodsetta_stone: await wideRes.text()
        };
...
function decodeMessage(messageElement) {
    ...
    // Normalize whitespace before cockblock detection (IRCCloud may insert non-breaking spaces)
...
function addEncodeButton() {
    ...
    button.onclick = (e) => {
        console.log('Cocktography: Button ONCLICK fired!');
        e.preventDefault();
        e.stopPropagation();
        encodeCurrentMessage();
    };
    ...
    console.log('Cocktography: Button added to body with handlers!');
}
...
function observeMessages() {
    ...
    getMessageElements().forEach(decodeMessage);
 ...
 */
function initUI() {
    console.log('Cocktography: Initializing UI...');
